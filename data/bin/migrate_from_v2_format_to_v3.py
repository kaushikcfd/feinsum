...
def record_into_db(cursor: sql.Cursor,
                   *,
                   ...
                   giga_op_info: str,
                   timestamp: str):
    from feinsum.sql_utils import (dump_index_to_length, dump_use_matrix,
                                   dump_value_to_dtype)
    ...
    cursor.execute("INSERT INTO FEINSUM_TIMING_FACTS"
                   ...
                   " VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                   (subscripts, index_to_length, use_matrix, value_to_dtype,
                    "NVIDIA_TITAN_V", transform_id, transform_params,
                    runtime_in_sec, compiler_version, giga_op_info, timestamp))


def main():
    conn = sql.connect(
        "file:/home/kgk2/projects/ceesd/feinsum"
        "/data/transform_archive_v2.sqlite?mode=ro",
        uri=True)
    cursor = conn.cursor()
    ...
    conn = sql.connect("transform_archive_v3.sqlite")
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE FEINSUM_TIMING_FACTS ("
    ...
    for fact in all_timing_facts:
        (subscripts, idx_to_len, use_matrix,
         val_to_dtype, transform_id, transform_params,
         runtime_in_sec, compiler_version, giga_op_info,
         timestamp) = fact
        input_subscripts = subscripts.split("->")[0]
        idx_to_len = json.loads(idx_to_len)
        val_to_dtype = {val: np.dtype(dtype)
                        for val, dtype in json.loads(val_to_dtype).items()}
        use_matrix = [
