...
def variant_1(t_unit, nwork_items_per_cell=4, ncells_per_workgroup=8):
    ...
    t_unit = lp.add_prefetch(t_unit,
                             ...
                             fetch_outer_inames=frozenset({x, i+"_inner", r+"_outer",
                                                           e+"_inner"}),
...
def variant_3(t_unit):
    ...
    t_unit = lp.add_prefetch(t_unit, D,
...
def paranumal_transform(t_unit, insn_match=None, kernel_name=None):
    ...
    t_unit = lp.precompute(t_unit, u, sweep_inames=[f"{e}_inner", j],
    ...
    t_unit = lp.precompute(t_unit, "subst", sweep_inames=[r],
                           precompute_outer_inames=frozenset({f"{e}_outer",
                                                              f"{e}_inner",
                                                              i}),
                           temporary_address_space=lp.AddressSpace.PRIVATE,
                           default_tag="unr")

    t_unit = lp.precompute(t_unit, J, sweep_inames=[x, r],
                           precompute_outer_inames=frozenset({f"{e}_outer",
                                                              f"{e}_inner",
                                                              i}),
                           temporary_address_space=lp.AddressSpace.PRIVATE,
