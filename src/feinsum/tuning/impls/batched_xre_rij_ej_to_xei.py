...
def transform(t_unit: lp.TranslationUnit,
              noutputs: int, ndim: int, ndof: int,
              n_e_per_wg: int, nwork_items_per_e: int,
              n_stmt_tile: int, i_tiles: int, j_tiles: int,
    ...
    if (
            ...
            + int(prftch_u_to_local)*ndof*n_e_per_wg*math.ceil(noutputs/n_stmt_tile)
            )*8e-3 > 47:
    ...
    ref_einsum = fnsm.batched_einsum("xre,rij,ej->xei",
    ...
    i_stmt_tile_to_fields = tuple(
        list(el)
        for el in chunked(us, math.ceil(noutputs/n_stmt_tile)))
    i_stmt_tile_to_outputs = tuple(
        list(el)
        for el in chunked(outputs, math.ceil(noutputs/n_stmt_tile)))

    ufetch_ids = tuple(tuple(ing("u_fetch_insn") for _ in fields)
    ...
    for istmt_tile in range(n_stmt_tile):
        ...
        prcmpt_j_redns = tuple(ing(f"prcmpt_{j}_redn")
                               for _ in i_stmt_tile_to_fields[istmt_tile])
        ...
        for subst_name, u, output in szip(subst_names,
        ...
        if prftch_u_to_local:
            ...
            for u, ufetch_id in zip(i_stmt_tile_to_fields[istmt_tile],
        ...
        for subst_name, prcmpt_j_redn in szip(subst_names,
                                              prcmpt_j_redns):
            t_unit = lp.precompute(
                t_unit, subst_name,
        ...
        t_unit = lp.realize_reduction(t_unit,
                                      insn_id_filter=frozenset(prcmpt_j_redns))
        ...
        for prcmpt_j_redn in prcmpt_j_redns:
            acc_name, = (
                t_unit[kernel_name].id_to_insn[prcmpt_j_redn].read_dependency_names()
                & set(t_unit[kernel_name].temporary_variables))
...
if __name__ == "__main__":
    ...
    expr = fnsm.batched_einsum("xre,rij,ej->xei",
                             operand_shapes=[(Ndim, Ndim, np.inf),
                                             (Ndim, Ndof, Ndof),
                                             (np.inf, Ndof)],
                             use_matrix=[
    ...
    if 1:
        ...
        bound_transform = partial(transform,
                                  ndim=Ndim, ndof=Ndof, noutputs=Nfields,
                                  n_e_per_wg=21,
