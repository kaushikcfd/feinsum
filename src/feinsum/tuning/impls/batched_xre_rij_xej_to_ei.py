...
@fnsm.tuning.transform_param(
    "n_stmt_tile", lambda e: IntParameter(1, e.noutputs))
@fnsm.tuning.transform_param(
...
def transform(t_unit: lp.TranslationUnit,
    ...
    if (math.ceil(noutputs/n_stmt_tile)*ndim*n_e_per_wg*math.ceil(ndof/j_tiles)
            + ndim*math.ceil(ndof/i_tiles)*math.ceil(ndof/j_tiles))*8e-3 > 47:
    ...
    for istmt_tile in range(n_stmt_tile):
        ...
        insn_ids = [
            ...
            if within(knl, insn) and (frozenset(outputs_in_tile)
        ...
        t_unit = lp.precompute(t_unit, D, [i_inner, r, j_inner],
                               ...
                               within=lp_match.And(
                                   (within,
                                    lp_match.Or(tuple(lp_match.Writes(out)
                                                      for out in outputs_in_tile)))),
    ...
    for istmt_tile in range(1, n_stmt_tile):
        ...
        for ifield, successor_field in enumerate(i_stmt_tile_to_fields[istmt_tile]):
            ...
            t_unit = lp.add_dependency(t_unit,
                                       ...
                                       depends_on=lp_match.And((within,
                                                                predecessor_match)))
