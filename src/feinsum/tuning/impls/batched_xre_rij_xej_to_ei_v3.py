...
def transform(
    ...
    if (
        noutputs * ndim * n_e_per_wg * math.ceil(ndof / j_tiles)
    ...
    insn_ids = [
        ...
        for insn in t_unit[kernel_name].instructions
        if within(t_unit[kernel_name], insn)
        and (frozenset(outputs) & insn.write_dependency_names())
    ]
    assert len(insn_ids) == len(outputs)

    subst_names = [vng("hoist_subst") for _ in us]
    hoisted_tmp_names = tuple(vng("J_times_u_fetch") for _ in us)
    D_fetch = vng(f"{D}_fetch")
    D_prftch_id = ing("D_fetch_insn")
    prcmpt_x_redn_ids = tuple(ing(f"prcmpt_{x}_redn") for _ in us)
    ...
    for subst_name, output in zip(subst_names, outputs, strict=True):
        knl = lp_utils.extract_multiplicative_terms_in_sum_reduction_as_subst(
            ...
            terms_filter=lambda term: (
                (get_dependencies(term) & knl.all_inames()) <= {r, e, j}
            ),
        )
    ...
    t_unit = lp.precompute(  # type: ignore[no-untyped-call]
        ...
        compute_insn_id=D_prftch_id,
        within=lp_match.And(
            (
                within,
                lp_match.Or(tuple(lp_match.Writes(out) for out in outputs)),
            )
        ),
        default_tag=None,
    )
    ...
    for isubst, (subst_name, tmp_name) in enumerate(
        zip(subst_names, hoisted_tmp_names, strict=True)
    ):
        t_unit = lp.precompute(  # type: ignore[no-untyped-call]
            t_unit,
            subst_name,
            ...
            compute_insn_id=prcmpt_x_redn_ids[isubst],
            temporary_address_space=lp.AddressSpace.LOCAL,
        )
