...
def transform(t_unit: lp.TranslationUnit,
    ...
    ref_einsum = fnsm.batched_einsum("ij,ej->ei",
                                   [(ndofs, ndofs),
                                    (np.inf, ndofs)],
                                   dtypes=np.float64,
                                   use_matrix=[
    ...
    t_unit = lp.split_iname(t_unit, iprftch_D, nworkitems_per_e,
                            inner_tag="l.0")
    t_unit = lp.split_iname(t_unit, jprftch_D, n_e_per_wg,
                            inner_tag="l.1")

    # }}}

    t_unit = lp.split_iname(t_unit, i, nworkitems_per_e,
                            outer_iname=i_outer, inner_iname=i_inner,
                            inner_tag="l.0", outer_tag="unr")

    t_unit = lp.add_inames_to_insn(
        t_unit,
        frozenset([i_inner]),
        insn_match=lp_match.Id(uprftch_insn_id)
    )
    t_unit = lp.realize_reduction(t_unit, insn_id_filter=frozenset([insn_id]))
    acc_name, = (t_unit[kernel_name].id_to_insn[insn_id].read_dependency_names()
...
if __name__ == "__main__":
    ...
    expr = fnsm.batched_einsum("ij,ej->ei",
                             [(Ndof, Ndof),
                              (np.inf, Ndof)],
                             dtypes=np.float64,
                             use_matrix=[
