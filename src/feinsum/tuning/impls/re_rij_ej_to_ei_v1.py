...
#         for (int r = 0; r < 3; r++) {
#           double Dprcmpt_priv = Dprcmpt[r,
#                                         i_tile*i_tilelen+lid(0),
#                                         j_tile*j_tilelen+j]
#           for (int e_inner = 0; e_inner < R_e; e_inner++)
#             du_acc[e_inner][i_tile][r] += Dprcmpt_priv
...
def transform(
    ...
    rj_prftch_D = vng("rjprftch_D")
    t_unit = lp.tag_inames(t_unit, {iprftch_D: "l.0"})
    t_unit = lp.join_inames(t_unit, [rprftch_D, jprftch_D], rj_prftch_D)
    t_unit = lp.split_iname(
        t_unit, rj_prftch_D, s_e, inner_tag="l.1", outer_tag="unr"
    )
    ...
    (du_update_insn_id,) = [
        ...
        for insn in t_unit[kernel_name].instructions
        if (
            D_fetch in insn.read_dependency_names()
            and u_priv_name in insn.read_dependency_names()
        )
    ]
...
if __name__ == "__main__":
    t_unit = lp.make_kernel(
        ...
        _u(_0, _1) := u[_0, _1]
        grad_x[e, i] = sum([r,j], _Jx(r,e)*_D(r,i,j)*_u(e,j))
        grad_y[e, i] = sum([r,j], _Jy(r,e)*_D(r,i,j)*_u(e,j))
        grad_z[e, i] = sum([r,j], _Jz(r,e)*_D(r,i,j)*_u(e,j))
        """,
