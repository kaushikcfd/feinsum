...
#       for (int r = 0; r < 3; r++)
#         for (int j = 0; j < T_j; j++)
#           tmp_Ju_priv[r][e_inner][j] = tmp_Ju[r][R_e * lid(1) + e_inner][j];
#     for (int i_tile = 0; i_tile < i_tiles; i_tile++) {
#      // Precompute Dprcmpt[:,:,:] <- D[:,
...
def transform(
    ...
    d_local_elems = ndim * i_tile_len * j_tile_len
    ...
    #
    # Dprcmpt[r, i_inner, j_inner] per (e_outer, i_tile, j_tile).
    # Shape: [ndim][i_tile_len][j_tile_len]
    ...
    t_unit = lp.tag_inames(t_unit, {iprftch_D: "l.0", rprftch_D: "unr"})
    ...
    t_unit = lp.precompute(  # type: ignore[no-untyped-call]
        t_unit,
        ju_priv_subst,
        sweep_inames=[r, e_r_e, j_inner_iname],
        precompute_inames=[ju_priv_r, ju_priv_e_r_e, ju_priv_j],
        precompute_outer_inames=frozenset(
            {e_outer, j_tile_iname, e_s_e, i_inner_iname}
        ),
        temporary_address_space=lp.AddressSpace.PRIVATE,
    ...
    t_unit = lp.tag_inames(
        t_unit,
        {ju_priv_r: "unr", ju_priv_e_r_e: "unr", ju_priv_j: "unr"},
