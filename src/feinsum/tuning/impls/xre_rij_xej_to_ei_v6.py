...
#       for (int i_tile = 0; i_tile < i_tiles; i_tile++) {
#         for (int j = 0; j < T_j; j++) {
#           double Dprcmpt_prftch = D[r][T_i*i_tile + lid(0)][T_j*j_tile + j];
#           for (int e_inner = 0; e_inner < R_e; e_inner++)
...
def transform(
    ...
    if (ndim * S_e * r_e * j_tile_len) * 8e-3 > 47:
    ...
    # {{{ Step 6: Realize redn over {r, j_inner} + privatize acc over i_tile,e_r_e
    ...
    (main_update_insn_id,) = [
        insn.id
        for insn in t_unit[kernel_name].instructions
        if ju_tmp_name in insn.read_dependency_names()
    ]

    # }}}

    # {{{ Step 7: Prefetch _tmp_Ju from LOCAL to PRIVATE
    #
